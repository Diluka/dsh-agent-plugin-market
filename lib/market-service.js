...
export function createMarketService({ runtime, hooks, onSkillsChanged }) {
  ...
  async function addMarket(args) {
    ...
    try {
      ...
      if (cfg) {
        cfg.markets = cfg.markets.filter((market) => market.id !== id)
        await runtime.saveConfig(cfg).catch(() => {})
      }
      await runtime.runProc(['rm', '-rf', id], marketsDir).catch(() => {})
      throw e
    }
  ...
  async function removeMarket(args) {
    ...
    for (const key of Object.keys(cfg.enabledStandaloneSkills)) {
      if (key.startsWith(id + '/')) delete cfg.enabledStandaloneSkills[key]
    }
  ...
  async function setSkillEnabled(args) {
    ...
    const cfg = await runtime.loadConfig()
    if (args.standalone) {
      if (enabled) cfg.enabledStandaloneSkills[fullName] = true
      else delete cfg.enabledStandaloneSkills[fullName]
    } else if (enabled) delete cfg.disabledSkills[fullName]
  ...
  async function setStandaloneSkillsEnabled(args) {
    ...
    for (const skill of skills) {
      if (enabled) cfg.enabledStandaloneSkills[skill.fullName] = true
      else delete cfg.enabledStandaloneSkills[skill.fullName]
    }
    await runtime.saveConfig(cfg)
    onSkillsChanged()
