...
export function createMarketService({ runtime, hooks, onSkillsChanged }) {
  ...
  async function getState() {
    ...
    for (const market of cfg.markets) {
      ...
      markets.push({ id: market.id, name: market.name, repo: market.repo, description: mp ? mp.description : '', refType: market.refType || 'default', ref: market.ref || null, manifestFound: !!mp, plugins })
    }
    return {
      hooksBridge: {
        available: hooks.available,
