...
/**
 * @param {{runtime: ReturnType<typeof import('./market-runtime.js').createMarketRuntime>, hooks: ReturnType<typeof import('./codex-hook-manager.js').createCodexHookManager>, onSkillsChanged: () => void, workspaces?: {list: () => Array<{id: string, title: string, path: string}>, get: (id: string) => {id: string, title: string, path: string} | undefined}}} options
 */
export function createMarketService({ runtime, hooks, onSkillsChanged, workspaces = { list: () => [], get: () => undefined } }) {
  ...
  function workspaceFor(args) {
    ...
    if (!workspace) throw new Error('工作区不存在或已移除')
    return workspace
  }
  ...
  async function addMarket(args) {
    ...
    /** @type {'default' | 'branch' | 'tag' | 'commit'} */
    const refType = ['branch', 'tag', 'commit'].includes(/** @type {any} */ (args.refType)) ? /** @type {'branch' | 'tag' | 'commit'} */ (args.refType) : 'default'
    ...
    let cfg
    try {
      /** @type {{id: string, name: string, repo: string, addedAt: number, refType?: 'branch' | 'tag' | 'commit', ref?: string}} */
  ...
  async function pullMarket(market) {
    ...
    try {
      ...
      try {
        ...
        resumeHooks = false
        console.error('[agent-plugin-market] keeping hooks suspended after failed pull for ' + market.id + ': ' + String((/** @type {any} */ (revokeError) && /** @type {any} */ (revokeError).message) || revokeError))
      }
      throw e
    } finally {
      if (resumeHooks) {
  ...
  async function setPluginHooksEnabled(args) {
    ...
    const hookInfo = await hooks.inspectCodexHooks(marketDir, pluginDir, /** @type {any} */ (await runtime.readPluginMeta(pluginDir, entry)))
    if (!hookInfo.found) throw new Error(hookInfo.error || '未发现可用的 Codex hooks 配置')

    cfg.hookApprovals[key] = { fingerprint: /** @type {string} */ (hookInfo.fingerprint), approvedAt: Date.now() }
    await runtime.saveConfig(cfg)
    await hooks.reconcile()
    onSkillsChanged()
  ...
  /**
   * @param {{workspaceId?: string}} [args]
   * @returns {Promise<{hooksBridge: {available: boolean, installCommand: string}, scope: object, workspaces: object[], markets: object[]}>}
   */
  async function getState(args = {}) {
     ...
     */
    const skillView = (skill, pluginKey) => {
      const override = workspaceOverride(workspaceConfig, 'pluginSkills', skill.fullName)
      return {
        name: skill.skillName,
        fullName: skill.fullName,
        description: skill.description,
        whenToUse: skill.whenToUse,
        globalEnabled: !cfg.disabledSkills[skill.fullName],
        workspaceOverride: override === undefined ? null : override,
        enabled: pluginSkillEnabled(cfg, workspaceConfig, pluginKey, skill.fullName),
      }
    }
    /** @param {{skillName: string, fullName: string, description: string, whenToUse: string | null}} skill */
    const standaloneSkillView = (skill) => {
      const override = workspaceOverride(workspaceConfig, 'standaloneSkills', skill.fullName)
      return {
        name: skill.skillName,
        fullName: skill.fullName,
        description: skill.description,
        whenToUse: skill.whenToUse,
        globalEnabled: !!cfg.enabledStandaloneSkills[skill.fullName],
        workspaceOverride: override === undefined ? null : override,
        enabled: standaloneSkillEnabled(cfg, workspaceConfig, skill.fullName),
      }
    }
    /** @type {any[]} */
    const markets = []
          ...
          if (!entry.unsupported) {
            try {
              ...
              if (meta.hookConfigs.codex) {
                const hookInfo = await hooks.inspectCodexHooks(marketDir, pluginDir, /** @type {any} */ (meta))
              ...
              view.error = String((/** @type {any} */ (e) && /** @type {any} */ (e).message) || e)
            }
          }
          plugins.push(view)
        }
    ...
    const workspaceViews = workspaces.list().map((item) => ({ id: item.id, title: item.title, path: item.path }))
    return {
      hooksBridge: {
        available: hooks.available,
        ...
        try {
          ...
          for (const market of cfg.markets) {
            ...
            try {
              ...
              console.error('[agent-plugin-market] auto update failed for ' + market.id + ': ' + String((/** @type {any} */ (e) && /** @type {any} */ (e).message) || e))
            }
          }
          if (!cancelled) {
            await hooks.reconcile()
          ...
          console.error('[agent-plugin-market] auto update error: ' + String((/** @type {any} */ (e) && /** @type {any} */ (e).message) || e))
        }
      })()
