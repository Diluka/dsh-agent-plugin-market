...
@media (max-width:640px){.apm-form{flex-wrap:wrap}.apm-form .apm-input{flex-basis:100%}.apm-plugin-topline{align-items:flex-start}.apm-plugin-actions{flex-wrap:wrap;justify-content:flex-end}.apm-plugin-summary{display:none}.apm-plugin-details{padding-left:0}}`))
      ...
      function MarketPage() {
        ...
        const catalogSummary = filter
          ? '匹配 ' + matchedPlugins + ' / ' + totalPlugins + ' 个插件，' + matchedSkills + ' / ' + totalSkills + ' 个技能'
          : catalogMode === 'all'
            ? '共 ' + totalPlugins + ' 个插件，' + totalSkills + ' 个技能'
            : (catalogMode === 'installed' ? '已安装 ' : '可用 ') + matchedPlugins + ' / ' + totalPlugins + ' 个插件，' + matchedSkills + ' / ' + totalSkills + ' 个技能'
        const children = []
        ...
        children.push(React.createElement('div', { className: 'apm-catalog-summary', role: 'status', 'aria-live': 'polite', key: 'catalog-summary' }, catalogSummary))
        ...
        for (const item of catalog) {
          ...
          for (const match of visibleMatches) {
            ...
            for (const skill of match.visibleSkills) {
              ...
              details.push(React.createElement('div', { className: 'apm-skill' + (isMatched ? ' match' : ''), key: skillId },
                plugin.installed
                ...
                React.createElement('div', { className: 'apm-skill-info' },
                  React.createElement('div', { className: 'apm-skill-name' + (isMatched ? ' match' : '') }, skill.name),
