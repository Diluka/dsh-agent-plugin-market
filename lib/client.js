...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * @typedef {{name: string, fullName?: string, description?: string, enabled?: boolean}} CatalogSkill
     * @typedef {{available?: boolean, found?: boolean, active?: boolean}} CatalogHooks
     * @typedef {{name: string, title?: string, installed?: boolean, unsupported?: boolean, skills?: CatalogSkill[], hooks?: CatalogHooks | null}} CatalogPlugin
     * @typedef {{plugins?: CatalogPlugin[], standaloneSkills?: CatalogSkill[]}} CatalogMarket
     * @typedef {{plugin: CatalogPlugin, visibleSkills: CatalogSkill[], matchingSkillIds: Set<string>}} CatalogMatch
     * @typedef {{market: CatalogMarket, plugins: CatalogPlugin[], standaloneSkills: CatalogSkill[], visibleStandaloneSkills: CatalogSkill[], matchingStandaloneSkillIds: Set<string>, matches: CatalogMatch[]}} CatalogResult
     * @typedef {{installedPlugins: number, activeSkills: number, activeHooks: number, availablePlugins: number, availableSkills: number, availableHooks: number}} CatalogStats
     * @typedef {{normalizedText: (value: unknown) => string, matchesText: (query: unknown, values: readonly unknown[]) => boolean, catalogForMarket: (market: CatalogMarket, query: string, catalogMode: CatalogMode) => CatalogResult, catalogStats: (markets: readonly CatalogMarket[]) => CatalogStats}} CatalogModel
     * @typedef {{connection: {isLoopback: boolean, rpc: {call: (route: string, endpoint: string, args: Record<string, unknown>) => Promise<{ok: boolean, value?: unknown, error?: {message: string}}>}}, slots: {inject: (name: string, callback: () => unknown) => unknown, register: (descriptor: object, render: () => unknown) => unknown}, effect: (callback: () => unknown) => unknown}} ClientPluginContext
     */
    ...
    /** @type {CatalogModel} */
    const catalogModel = { normalizedText, matchesText, catalogForMarket, catalogStats }
...
@media (max-width:640px){.apm-form{flex-wrap:wrap}.apm-form .apm-input{flex-basis:100%}.apm-plugin-topline{align-items:flex-start}.apm-plugin-actions{flex-wrap:wrap;justify-content:flex-end}.apm-plugin-summary{display:none}.apm-plugin-details,.apm-standalone-details{padding-left:0}}`))
      ...
      function MarketPage() {
        ...
        const [expandedStandaloneSkills, setExpandedStandaloneSkills] = React.useState({})
