...
.apm-loading{color:var(--dsw-alias-label-secondary);padding:16px 0}`)
      ...
      function MarketPage() {
        ...
        const act = React.useCallback(async (name, fn) => {
          ...
          try {
            const res = await fn()
            if (!res || !res.ok) setError((res && res.error) || (name + ' 失败'))
            else await refresh()
          } catch (e) { setError(String((e && e.message) || e)) }
          setBusy(null)
        }, [refresh])
        ...
        children.push(React.createElement('div', { className: 'apm-hint' },
          '将 git 仓库作为 agent 插件市场：添加后克隆到 .dsh/agent-plugin-market，安装插件即原地加载其技能（兼容 Codex / Claude 的 SKILL.md 格式）。'))
        ...
        children.push(React.createElement('div', { className: 'apm-form' },
          React.createElement('input', {
            ...
            onKeyDown: (e) => {
              if (e.key === 'Enter' && repo.trim() && busy === null) {
                act('add-market', () => apiCall('add-market', { repo: repo.trim() })).then(() => { if (!error) setRepo('') })
              }
            },
          }),
          React.createElement(Btn, {
            ...
            onClick: () => act('add-market', () => apiCall('add-market', { repo: repo.trim() })).then(() => { if (!error) setRepo('') }),
