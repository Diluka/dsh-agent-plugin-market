...
.apm-scope{margin:0 0 14px}
.apm-scope-head{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin:0 0 8px}
...
.apm-scope-options{display:flex;align-items:center;gap:8px;flex-wrap:wrap}
.apm-scope-chip{appearance:none;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary);border-radius:999px;padding:5px 12px;font:inherit;font-weight:600;cursor:pointer;max-width:260px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.apm-scope-chip:hover:not(:disabled){border-color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-layer-2)}
.apm-scope-chip.active{border-color:var(--dsw-alias-brand-primary);background:var(--dsw-alias-brand-primary);color:var(--dsw-alias-bg-base)}
.apm-scope-chip:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:2px}
.apm-scope-chip:disabled{opacity:.65;cursor:default}
...
@media (max-width:640px){.apm-root{min-width:0;overflow-wrap:anywhere}.apm-form{flex-wrap:wrap}.apm-form .apm-input{flex-basis:100%}.apm-scope-head{align-items:stretch}.apm-scope-label,.apm-scope-note{width:100%}.apm-scope-options{display:grid;grid-template-columns:1fr}.apm-scope-chip{max-width:none;text-align:left}.apm-scope-mode{min-width:0;max-width:100%;white-space:normal}.apm-card{padding:12px;overflow:hidden}.apm-card-head,.apm-plugin-topline{align-items:flex-start;flex-wrap:wrap}.apm-market-actions,.apm-plugin-actions{width:100%;flex-wrap:wrap;justify-content:flex-start;margin-left:0}.apm-plugin-summary{display:none}.apm-plugin-details,.apm-standalone-details{padding-left:0}.apm-skill{flex-wrap:wrap}.apm-skill-state{white-space:normal}}`))
      ...
      /** @param {any} props */
      function WorkspaceScopeTabs(props) {
        const buttons = [React.createElement('button', {
          key: 'global',
          type: 'button',
          className: 'apm-scope-chip' + (!props.workspaceId ? ' active' : ''),
          disabled: props.disabled,
          'aria-pressed': !props.workspaceId,
        ...
        for (const workspace of Array.isArray(props.workspaces) ? props.workspaces : []) {
          ...
          const active = props.workspaceId === workspace.id
          const label = String(workspace.title || workspace.path || workspace.id)
          buttons.push(React.createElement('button', {
            key: workspace.id,
            type: 'button',
            className: 'apm-scope-chip' + (active ? ' active' : ''),
        ...
        return React.createElement('div', { className: 'apm-scope-options' }, ...buttons)
      }
      ...
      function MarketPage() {
        ...
        const [workspaceId, setWorkspaceId] = React.useState(/** @type {string | null} */ (null))
        ...
        React.useEffect(() => {
          ...
          refresh()
        }, [refresh, workspaceFingerprint, workspaceId])
        ...
        children.push(React.createElement('section', { className: 'apm-scope', key: 'scope', 'aria-label': '配置作用域' },
          React.createElement('div', { className: 'apm-scope-head' },
            ...
            isWorkspaceScope
              ...
              : React.createElement('span', { className: 'apm-scope-note' }, '全局默认会作为所有工作区的继承值'),
            isWorkspaceScope
              ...
              : null),
          React.createElement(WorkspaceScopeTabs, {
