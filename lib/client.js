/// <reference path="../types/client-bundle.d.ts" />
...
const inject = ['connection', 'slots', 'workspaces', 'configForms']

window.__ModuleLoader__.load({
  id: PACKAGE_ID,
  factory: (require) => {
    const React = require('react')
    /** @type {ClientUiPrimitivesModule | null} */
    ...
    /**
     * @param {WorkspaceListSnapshot | null | undefined} workspaceList
     * @returns {WorkspaceItem[]}
     */
    function workspaceItemsFromSnapshot(workspaceList) {
      const source = workspaceList?.items
      const items = Array.isArray(source) ? source : []
      return items.flatMap((workspace) => (
        workspace && typeof workspace.workspaceId === 'string' && typeof workspace.title === 'string' && typeof workspace.path === 'string'
          ? [{ id: workspace.workspaceId, title: workspace.title, path: workspace.path }]
          : []
      ))
    }
    ...
    /**
     * @param {{items?: unknown, anchor?: {props?: {'aria-label'?: unknown}}} | null | undefined} menuProps
     * @param {readonly WorkspaceItem[]} workspaces
     */
    ...
    const styles = `.apm-root{font-size:13px;color:var(--dsw-alias-label-primary);line-height:1.5}
        .apm-feature-card{border:.5px solid var(--dsw-alias-border-l4);background:var(--dsw-alias-bg-layer-3);border-radius:16px;list-style:none;transition:border-color .16s,background .16s}
        .apm-feature-card:hover{border-color:var(--dsw-alias-label-dimmed)}
        .apm-feature-card.open{background:var(--dsw-alias-bg-layer-2);border-color:var(--dsw-alias-label-dimmed)}
        .apm-feature-header{appearance:none;width:100%;font:inherit;color:inherit;text-align:left;cursor:pointer;background:transparent;border:0;border-radius:12px;display:flex;align-items:center;gap:12px;padding:14px 16px}
        .apm-feature-header:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:-2px}
        .apm-feature-head-text{display:flex;flex-direction:column;flex:1;gap:4px;min-width:0}
        .apm-feature-name{color:var(--dsw-alias-label-primary);font-size:15px;font-weight:600;line-height:1.4}
        .apm-feature-description{color:var(--dsw-alias-label-tertiary);font-size:13px;line-height:1.5}
        .apm-feature-chevron{color:var(--dsw-alias-label-tertiary);flex:none;transition:transform .16s}
        .apm-feature-card.open .apm-feature-chevron{transform:rotate(180deg)}
        .apm-feature-body{border-top:.5px solid var(--dsw-alias-border-l2);margin:0 16px;padding-bottom:8px}
        .apm-feature-group{color:var(--dsw-alias-label-primary);font-size:13px;font-weight:600;line-height:1.5;margin:12px 0 0}
        .apm-feature-field{display:flex;flex-direction:column;gap:6px;padding:12px 0}
        .apm-feature-field+.apm-feature-field{border-top:.5px solid var(--dsw-alias-border-l2)}
        .apm-feature-field-head{display:flex;align-items:center;gap:8px}
        .apm-feature-label{flex:1;min-width:0;color:var(--dsw-alias-label-primary);font-size:13px;font-weight:500;line-height:1.5}
        .apm-feature-hint{color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:1.5;margin:0}
        .apm-feature-footer{border-top:.5px solid var(--dsw-alias-border-l2);padding:12px 0 4px}
        .apm-feature-error{color:var(--dsw-alias-label-error);font-size:12px;line-height:1.5;margin:4px 0 0}
        ...
        .apm-ref-editor{margin:8px 0 12px;align-items:center;flex-wrap:wrap}
        .apm-ref-editor .apm-hint{margin:0}
        ...
        .apm-repo{appearance:none;border:0;background:transparent;display:inline-flex;align-items:center;justify-content:center;color:var(--dsw-alias-label-secondary);flex:none;padding:4px;margin:0;border-radius:4px;cursor:pointer}
        .apm-repo:hover{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-layer-2)}
        .apm-repo:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:2px}
        .apm-tag{flex:none}
    ...
    /** @typedef {{variant?: 'primary' | 'danger', disabled?: boolean, onClick?: () => void, title?: string, ariaLabel?: string, 'aria-label'?: string, children?: ReactNode}} BtnProps */
    ...
    /** @typedef {PrimitiveInputProps} TextInputProps */
    ...
    /** @typedef {{title?: string, children?: ReactNode}} TagProps */
    ...
    /** @typedef {{value?: string, disabled?: boolean, onChange(value: string): void}} RefTypeMenuProps */
    /** @param {RefTypeMenuProps} props */
    function RefTypeMenu(props) {
      ...
      return React.createElement(UI.Menu, {
        ...
        anchor: React.createElement(UI.Button, {
          ...
          size: 'sm',
          disabled: props.disabled,
          icon: React.createElement(UI.IconBranchOutlineMedium, { size: 16 }),
    ...
    function refTypeLabel(refType) {
      if (refType === 'branch') return '分支'
      if (refType === 'tag') return '标签'
      if (refType === 'commit') return '提交'
      return '默认分支'
    }

    /** @param {string | null | undefined} refType */
    function refPlaceholder(refType) {
      if (refType === 'branch') return '分支名，如 main'
      if (refType === 'tag') return '标签名，如 v1.0.0'
      return 'commit id，如 1a2b3c4d'
    }

    /** @param {CatalogMarket} market */
    function marketRefText(market) {
      const refType = market.refType || 'default'
      return refType === 'default' ? refTypeLabel(refType) : refTypeLabel(refType) + ': ' + String(market.ref || '')
    }

    /** @typedef {{workspaceId?: string | null, workspaces?: WorkspaceItem[], disabled?: boolean, onChange(workspaceId: string | null): void}} WorkspaceScopeMenuProps */
    /** @param {WorkspaceScopeMenuProps} props */
    ...
    /** @typedef {{override?: boolean | null, globalEnabled?: boolean, disabled?: boolean, onChange(mode: string): void}} WorkspaceOverrideMenuProps */
    /** @param {WorkspaceOverrideMenuProps} props */
    ...
    /** @typedef {{on?: boolean, label?: string, title?: string, disabled?: boolean, onClick?: () => void}} SwitchProps */
    ...
    /** @typedef {{className?: string, leading?: ReactNode, matched?: boolean, name?: ReactNode, description?: ReactNode}} CatalogItemProps */
    /** @param {CatalogItemProps} props */
     ...
     * @param {(workspace: WorkspaceItem) => boolean} canConfigure
    ...
    function installWorkspaceMenuBridge(ctx, workspaceConfigStore, canConfigure, setScanner) {
      ...
      const rememberWorkspaceButton = (target) => {
        ...
        const button = element ? element.closest('button[aria-label]') : null
        ...
        activeButton = button instanceof HTMLButtonElement ? button : null
      }
      /** @param {Element} menu */
      ...
      const injectRow = (menu) => {
        ...
        item.onclick = /** @param {Event} event */ (event) => {
          ...
          workspaceConfigStore.open(workspace)
    ...
    /**
     * @param {{workspaceConfigStore: ReturnType<typeof createWorkspaceConfigStore>, Btn: typeof Btn, MarketPage: WorkspaceMarketPage}} options
     */
    function createWorkspaceConfigOverlay(options) {
      return function WorkspaceConfigOverlay() {
        ...
        return React.createElement('div', {
          ...
          onMouseDown: /** @param {ReactModalBackdropMouseEvent} event */ (event) => {
          ...
          onMouseDown: /** @param {ReactModalContentMouseEvent} event */ (event) => { event.stopPropagation() },
        },
          ...
          React.createElement('div', { className: 'apm-modal-body' },
            React.createElement(options.MarketPage, { workspace }))))
      }
    }
    ...
    function createSettingsUi(ctx, config) {
      ...
      const apiCall = async (name, args = {}) => {
        ...
        return { ok: true, data: /** @type {T} */ (result.value) }
      }

      /**
       * @param {object} [args]
       * @returns {Promise<ApiResult<MarketState>>}
       */
      const getState = (args = {}) => apiCall('get-state', args)

      const UI = getUIPrimitives()
      ctx.effect(installStyles)

      /** @typedef {{disabled: boolean, confirming: boolean, onClick: () => void}} WorkspaceResetButtonProps */
      /** @param {WorkspaceResetButtonProps} props */
      ...
      /** @typedef {{fixedWorkspace?: WorkspaceItem | null, scope: ConfigScope, isWorkspaceScope: boolean, workspaceId: string | null, workspaces: WorkspaceItem[], disabled: boolean, confirming: boolean, onClearOverrides: () => void, onScopeChange: (workspaceId: string | null) => void}} ScopeControlsProps */
      /** @param {ScopeControlsProps} props */
      ...
      /** @typedef {{repo: string, refType: string, refText: string, busy: string | null, disabled: boolean, onRepoChange: (repo: string) => void, onRefTypeChange: (refType: string) => void, onRefTextChange: (ref: string) => void, onSubmit: () => void | Promise<void>}} AddMarketFormProps */
      /** @param {AddMarketFormProps} props */
        ...
        return React.createElement(React.Fragment, null,
          ...
          React.createElement('div', { className: 'apm-form apm-form-ref' },
            ...
            props.refType !== 'default'
              ? React.createElement(TextInput, {
                  className: 'apm-input',
                  placeholder: refPlaceholder(props.refType),
                  'aria-label': '市场 Git 引用',
                  value: props.refText,
                  disabled: props.disabled,
                  onChange: (e) => props.onRefTextChange(e.target.value),
                })
              : React.createElement('span', { className: 'apm-hint' }, '使用仓库默认分支（与 GitHub 默认一致），启动时自动拉取更新')))
      }

      /** @typedef {{refType: string, refText: string}} MarketRefEdit */
      /** @typedef {{edit: MarketRefEdit, disabled: boolean, saving: boolean, onRefTypeChange: (value: string) => void, onRefTextChange: (value: string) => void, onSubmit: () => void | Promise<void>, onCancel: () => void}} MarketRefEditorProps */
      /** @param {MarketRefEditorProps} props */
      function MarketRefEditor(props) {
        const needsRef = props.edit.refType !== 'default'
        return React.createElement('div', { className: 'apm-form apm-ref-editor' },
          React.createElement(RefTypeMenu, {
            value: props.edit.refType,
            disabled: props.disabled,
            onChange: props.onRefTypeChange,
          }),
          needsRef
            ? React.createElement(TextInput, {
                className: 'apm-input',
                placeholder: refPlaceholder(props.edit.refType),
                'aria-label': '市场 Git 引用',
                value: props.edit.refText,
                disabled: props.disabled,
                onChange: (e) => props.onRefTextChange(e.target.value),
                onKeyDown: (e) => { if (e.key === 'Enter') props.onSubmit() },
              })
            : React.createElement('span', { className: 'apm-hint' }, '切回仓库默认分支，启动时继续自动拉取更新'),
          React.createElement(Btn, {
            variant: 'primary',
            disabled: props.disabled || (needsRef && !String(props.edit.refText).trim()),
            onClick: props.onSubmit,
          }, props.saving ? '保存中…' : '保存引用'),
          React.createElement(Btn, {
            disabled: props.disabled,
      ...
      /** @typedef {{market: CatalogMarket, busy: string | null, refEdit?: MarketRefEdit, confirmingRemove: boolean, onEditRef: () => void, onUpdate: () => void, onRemove: () => void}} MarketActionsMenuProps */
      /** @param {MarketActionsMenuProps} props */
      function MarketActionsMenu(props) {
        const [open, setOpen] = React.useState(false)
        const busy = props.busy !== null
        const label = props.busy === 'update-' + props.market.id
          ? '更新中…'
          : props.busy === 'remove-' + props.market.id
            ? '移除中…'
            : props.confirmingRemove
              ? '确认移除？'
              : '管理'
        const items = [
          { id: 'edit-ref', label: props.refEdit ? '正在编辑引用' : '修改引用', disabled: !!props.refEdit },
          { id: 'update', label: '更新市场' },
          { id: 'remove', label: props.confirmingRemove ? '确认移除市场' : '移除市场', danger: true },
        ]
        return React.createElement(UI.Menu, {
          open,
          anchor: React.createElement(UI.Button, {
            variant: 'outline',
            size: 'sm',
            className: 'apm-btn' + (props.confirmingRemove ? ' danger' : ''),
            disabled: busy,
            onClick: () => setOpen(!open),
          }, label),
          items,
          onSelect: (id) => {
            ...
            if (busy) return
            if (id === 'edit-ref') {
              if (!props.refEdit) props.onEditRef()
            } else if (id === 'update') {
              props.onUpdate()
            } else if (id === 'remove') {
              props.onRemove()
      ...
      /** @typedef {{query: string, mode: CatalogMode, disabled: boolean, onQueryChange: (query: string) => void, onModeChange: (mode: CatalogMode) => void}} CatalogToolbarProps */
      /** @param {CatalogToolbarProps} props */
      function CatalogToolbar(props) {
        ...
        return React.createElement('div', { className: 'apm-catalog-toolbar' },
          React.createElement(TextInput, {
            ...
            icon: React.createElement(UI.IconSearchOutlineMedium, { size: 16 }),
      ...
      /** @typedef {{total: number, visible: number, disabled: boolean, moreLabel?: (remaining: number) => string, collapseLabel?: string, onMore: () => void, onCollapse: () => void}} CatalogMoreActionsProps */
      /** @param {CatalogMoreActionsProps} props */
      ...
      /** @typedef {{skill: CatalogSkill, matched?: boolean, isWorkspaceScope: boolean, scope: ConfigScope, busy: string | null, act: (name: string, fn: () => Promise<ActionResult>) => Promise<boolean>, pluginActive?: boolean, standalone?: boolean}} SkillRowProps */
      /** @param {SkillRowProps} props */
      function SkillRow(props) {
        ...
        if (props.isWorkspaceScope) {
          leading = React.createElement(WorkspaceOverrideMenu, {
            ...
            disabled,
            onChange: /** @param {string} mode */ (mode) => props.act(skillBusy, () => apiCall('set-workspace-skill-enabled', {
          ...
          leading = React.createElement(Switch, {
            ...
            onClick: () => props.act(skillBusy, () => apiCall('set-skill-enabled', {
              fullName: skill.fullName,
      ...
      /** @typedef {{hook?: CatalogHooks | null, isWorkspaceScope: boolean, pluginKey: string, plugin: CatalogPlugin, market: CatalogMarket, busy: string | null, confirmHooks: string | null, setConfirmHooks: (pluginKey: string | null) => void, act: (name: string, fn: () => Promise<ActionResult>) => Promise<boolean>}} HooksRowProps */
      /** @param {HooksRowProps} props */
      ...
      /** @typedef {{market: CatalogMarket, plugin: CatalogPlugin, pluginKey: string, scope: ConfigScope, isWorkspaceScope: boolean, busy: string | null, act: (name: string, fn: () => Promise<ActionResult>) => Promise<boolean>}} PluginActionProps */
      /** @param {PluginActionProps} props */
      ...
      /** @typedef {{market: CatalogMarket, match: CatalogPluginMatch, scope: ConfigScope, isWorkspaceScope: boolean, busy: string | null, confirmHooks: string | null, setConfirmHooks: (pluginKey: string | null) => void, act: (name: string, fn: () => Promise<ActionResult>) => Promise<boolean>, open: boolean, onToggle: () => void}} PluginSectionProps */
      /** @param {PluginSectionProps} props */
      function PluginSection(props) {
        ...
        const showHooks = !!((pluginActive || props.isWorkspaceScope) && plugin.hooks && (plugin.hooks.found || plugin.hooks.error))
        const canExpand = !plugin.unsupported && (skillCount > 0 || showHooks)
        ...
        pluginContent.push(React.createElement('div', { className: 'apm-plugin-topline', key: 'head' },
          React.createElement('div', { className: 'apm-plugin-disclosure' },
            React.createElement(UI.DisclosureRow, {
              icon: React.createElement(UI.IconSkillOutlineMedium, { size: 16 }),
      ...
      /** @typedef {{item: CatalogMarketView, market: CatalogMarket, scope: ConfigScope, isWorkspaceScope: boolean, busy: string | null, act: (name: string, fn: () => Promise<ActionResult>) => Promise<boolean>, open: boolean, limit: number, limitKey: string, setMarketLimits: MarketLimitSetter, onToggle: () => void}} StandaloneSkillsSectionProps */
      /** @param {StandaloneSkillsSectionProps} props */
      ...
      async function refreshConfigurableWorkspaces() {
        ...
        configurableWorkspaceRefresh = getState({}).then((res) => {
      ...
      function MarketPage(props) {
        ...
        const [state, setState] = React.useState(/** @type {MarketState | null} */ (null))
        ...
        const [marketRefEdits, setMarketRefEdits] = React.useState(/** @type {Record<string, MarketRefEdit>} */ ({}))
        const [repoCopyStatus, setRepoCopyStatus] = React.useState(/** @type {Record<string, 'copied' | 'failed'>} */ ({}))
        const repoCopyResetTimer = React.useRef(/** @type {ReturnType<typeof setTimeout> | null} */ (null))
        ...
        const workspaceItems = state && Array.isArray(state.workspaces)
          ? state.workspaces.filter((workspace) => workspace && typeof workspace.id === 'string' && typeof workspace.title === 'string' && typeof workspace.path === 'string')
          : rawWorkspaceItems.filter((workspace) => canConfigureWorkspace(workspace))
        const workspaceFingerprint = workspaceModel.fingerprint(rawWorkspaceItems)

        React.useEffect(() => () => {
          if (repoCopyResetTimer.current) clearTimeout(repoCopyResetTimer.current)
        }, [])
        ...
        const refresh = React.useCallback(async () => {
          try {
            const res = await getState(workspaceId ? { workspaceId } : {})
            if (res && res.ok) {
        ...
        /** @param {CatalogMarket} market */
        const startMarketRefEdit = (market) => {
          setMarketRefEdits((current) => ({
            ...current,
            [market.id]: { refType: market.refType || 'default', refText: market.ref || '' },
          }))
        }

        /** @param {string} marketId */
        const cancelMarketRefEdit = (marketId) => {
          setMarketRefEdits((current) => {
            const next = { ...current }
        ...
        /**
         * @param {string} marketId
         * @param {Partial<MarketRefEdit>} patch
         */
        const updateMarketRefEdit = (marketId, patch) => {
          setMarketRefEdits((current) => ({ ...current, [marketId]: { ...(current[marketId] || { refType: 'default', refText: '' }), ...patch } }))
        }

        /** @param {CatalogMarket} market */
        const submitMarketRef = async (market) => {
          const edit = marketRefEdits[market.id]
          if (!edit || busy !== null) return
          const nextRefType = edit.refType || 'default'
          const nextRef = nextRefType === 'default' ? '' : String(edit.refText).trim()
          if (nextRefType !== 'default' && !nextRef) return
          const currentRefType = market.refType || 'default'
          const currentRef = currentRefType === 'default' ? '' : String(market.ref || '')
          if (currentRefType === nextRefType && currentRef === nextRef) {
            cancelMarketRefEdit(market.id)
            setInfo('市场引用未变化')
            return
          }
          const saved = await act('market-ref-' + market.id, () => apiCall('set-market-ref', {
            marketId: market.id,
            refType: nextRefType,
            ref: nextRef,
          }))
        ...
        /** @param {CatalogMarket} market */
        const copyMarketRepo = async (market) => {
          const repoText = String(market.repo || '').trim()
          if (!repoText) return
          if (repoCopyResetTimer.current) clearTimeout(repoCopyResetTimer.current)
          try {
            await navigator.clipboard.writeText(repoText)
            setRepoCopyStatus({ [market.id]: 'copied' })
          } catch {
            setRepoCopyStatus({ [market.id]: 'failed' })
          }
          repoCopyResetTimer.current = setTimeout(() => setRepoCopyStatus({}), 1600)
        }
        ...
        if (!isWorkspaceScope) {
          children.push(React.createElement(AddMarketForm, {
            ...
            onRefTypeChange: (value) => setRefType(value),
        ...
        for (const item of catalog) {
          ...
          const refEdit = marketRefEdits[market.id]
          const repoCopy = repoCopyStatus[market.id]
          ...
          card.push(React.createElement('div', { className: 'apm-card-head', key: 'head' },
            ...
            React.createElement('div', { className: 'apm-market-actions' },
              React.createElement(UI.Tooltip, {
                label: repoCopy === 'copied' ? '已复制' : repoCopy === 'failed' ? '复制失败' : market.repo,
                side: 'bottom',
                maxWidth: 360,
                children: /** @type {TooltipAnchorElement} */ (
                  React.createElement('button', {
                    type: 'button',
                    className: 'apm-repo',
                    title: '复制仓库地址',
                    'aria-label': '复制仓库地址：' + market.repo,
                    onClick: () => copyMarketRepo(market),
                  }, React.createElement(UI.IconLinkOutlineMedium, { size: 16 }))),
              }),
              ...
              React.createElement(Tag, { key: 'ref', title: marketRefText(market) }, marketRefText(market)),
              !isWorkspaceScope
                ? React.createElement(MarketActionsMenu, {
                    market,
                    busy,
                    refEdit,
                    confirmingRemove: confirmRemove === market.id,
                    onEditRef: () => {
                      setConfirmRemove(null)
                      startMarketRefEdit(market)
                    },
                    onUpdate: () => {
                      setConfirmRemove(null)
                      act('update-' + market.id, () => apiCall('update-market', { marketId: market.id }))
                    },
                    onRemove: () => {
                ...
                : null)))
          if (!isWorkspaceScope && refEdit) {
            card.push(React.createElement(MarketRefEditor, {
              key: 'ref-editor',
              edit: refEdit,
              disabled: busy !== null,
              saving: busy === 'market-ref-' + market.id,
              onRefTypeChange: (value) => updateMarketRefEdit(market.id, { refType: value }),
              onRefTextChange: (value) => updateMarketRefEdit(market.id, { refText: value }),
              onSubmit: () => submitMarketRef(market),
              onCancel: () => cancelMarketRefEdit(market.id),
            }))
          }
    ...
    /** @param {PluginBundleConfigProps} props */
    function FeatureConfigCard({ view, form }) {
      const subscribe = React.useCallback((/** @type {() => void} */ listener) => form.subscribe(listener), [form])
      ...
      if (view === 'summary') {
        return React.createElement('span', null, '配置市场工具及其系统提示词。')
      }
      if (view !== 'page' || snapshot.status !== 'ready' || !snapshot.value) return null
      const value = snapshot.value
      const UI = getUIPrimitives()
      /** @param {'tools' | 'systemPrompt'} field @param {boolean} next */
      async function save(field, next) {
        setBusy(true)
        setError('')
        try {
          const accepted = await form.set(field, next)
          if (!accepted) setError('设置已变化或当前只读，请重新打开此页后重试。')
        } catch (e) { setError(e instanceof Error ? e.message : String(e)) }
        finally { setBusy(false) }
      }
      /** @param {'tools' | 'systemPrompt'} field @param {string} label @param {string} hint */
      function row(field, label, hint) {
        return React.createElement('div', { className: 'apm-feature-field' },
          React.createElement('div', { className: 'apm-feature-field-head' },
            React.createElement('span', { className: 'apm-feature-label' }, label),
            React.createElement(UI.Switch, {
              label,
              checked: field === 'systemPrompt' ? value.tools && value.systemPrompt : value.tools,
              disabled: busy || !snapshot.writable || (field === 'systemPrompt' && !value.tools),
              onChange: (next) => { void save(field, next) },
            })),
          React.createElement('p', { className: 'apm-feature-hint' }, hint))
      }
      // Match the native PluginCard's stacked header; its internal component
      // and CSS-module classes are not public exports. Keep the adapter local.
      return React.createElement('div', { className: 'apm-feature-card' + (open ? ' open' : '') },
        React.createElement('button', {
          type: 'button',
          className: 'apm-feature-header',
          'aria-expanded': open,
          'aria-label': (open ? '收起' : '展开') + '：Agent 插件市场',
          onClick: () => setOpen(!open),
        },
        React.createElement('span', { className: 'apm-feature-head-text' },
          React.createElement('span', { className: 'apm-feature-name' }, 'Agent 插件市场'),
          React.createElement('span', { className: 'apm-feature-description' }, '配置市场工具及其系统提示词。')),
        React.createElement(UI.IconChevronDownOutlineMedium, { className: 'apm-feature-chevron', size: 14 })),
        open ? React.createElement('div', { className: 'apm-feature-body' },
          React.createElement('h4', { className: 'apm-feature-group' }, '功能'),
          row('tools', '启用工具', '默认启用。允许模型查看市场状态、修改工作区插件与技能覆盖；不影响技能加载、hooks 或市场管理界面。'),
          row('systemPrompt', '注入系统提示词', '默认随工具启用；工具未启用时默认也禁用，不注入工具提示。可独立关闭；home 会话不注入。'),
          React.createElement('div', { className: 'apm-feature-footer' },
            React.createElement('p', { className: 'apm-feature-hint', role: 'status' }, busy ? '保存中…' : snapshot.writable ? '保存后即时生效。' : '当前设置只读。'),
            error ? React.createElement('p', { className: 'apm-feature-error', role: 'alert' }, error) : null)) : null)
    }

    /**
     * Registers the bundle configuration and market settings page in the browser client.
     * @param {ClientContext} ctx
     */
    function apply(ctx) {
      const form = /** @type {MarketFeatureForm} */ (ctx.configForms.get(PACKAGE_ID))
      ctx.slots.inject('plugins.bundle.config', () => ctx.slots.register(
        { name: 'plugins.bundle.config', key: PACKAGE_ID },
        (props) => React.createElement(FeatureConfigCard, { ...props, form }),
      ))
