...
const inject = ['connection', 'slots', 'workspaces', 'settingsScope']
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** @param {{scope: MarketFeatureScope}} props */
    function FeatureConfigCard({ scope }) {
      const subscribe = React.useCallback((/** @type {() => void} */ listener) => scope.subscribe(listener), [scope])
      ...
      async function save(field, next) {
        setBusy(true)
        setError('')
        try { await scope.set(field, next) }
      ...
      return React.createElement('li', { className: 'apm-feature-card' + (open ? ' open' : '') },
        React.createElement('button', {
          type: 'button',
        ...
        React.createElement(UI.IconChevronDownOutline14, { className: 'apm-feature-chevron' })),
     ...
     */
    function apply(ctx) {
      const scope = ctx.settingsScope.bind({ namespace: 'agent-plugin-market' })
      ctx.slots.inject('settings.plugin.item', () => ctx.slots.register(
        { name: 'settings.plugin.item', key: 'agent-plugin-market' },
        () => React.createElement(FeatureConfigCard, { scope }),
      ))
