...
import { workspacePathForCwd } from './market-tools.js'
import { installMarketFeatures } from './market-features.js'
export { Config } from './market-features.js'

// Connection registers custom RPC channels on the caller Fiber's webServer.
/** @type {readonly ['skills', 'fs', 'subprocess', 'connection', 'webServer']} */
export const inject = ['skills', 'fs', 'subprocess', 'connection', 'webServer']

export { workspacePathForCwd }
...
export async function apply(ctx, config) {
  const { skills, fs, subprocess, connection } = ctx
  ...
  const runtime = createMarketRuntime({ fs, subprocess, dshHome: dshHomePath() })
  ...
  await installMarketFeatures(ctx, config, { service, runtime, workspaces })
  ...
  const rpcHandlers = new Map(/** @type {Array<[string, RpcHandler]>} */ ([
    ...
    ['set-market-ref', service.setMarketRef],
  ...
  /**
   * @param {unknown} error
   * @returns {import('@deepseek-ai/dsh-client-connection').ConnectionRpcResult<never>}
   */
