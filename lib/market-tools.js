...
/**
 * Short model guidance; suppressed for the same home-path sessions as the tools.
 * @param {MarketRuntime} runtime
 * @param {HostAgent | undefined} agent
 */
export function marketToolsPrompt(runtime, agent) {
  if (isHomeCwd(runtime, agentCwd(agent))) return ''
  return 'Use agent_market_info first to read market state and exact market, plugin, and skill identifiers. Use agent_market_set_plugin or agent_market_set_skill only for the requested workspace override: inherit restores the global setting, enabled/disabled override it in that workspace. These tools do not install/uninstall plugins, change global settings, or authorize hooks. Omit workspace_id to use the current session workspace; home-path sessions cannot use market tools.'
}
