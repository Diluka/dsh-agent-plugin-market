...
/**
 * @param {{fs: {lstat: (path: string) => Promise<{type: string} | null>, resolve: (path: string) => Promise<{displayPath: string, targetKey: string}>, stat: (target: object) => Promise<{type: string, version?: unknown} | null>, readText: (target: object) => Promise<string>, writeText: (target: object, content: string, expected?: object, signal?: unknown, sandboxPolicy?: {mode: 'read-only' | 'workspace-write' | 'danger-full-access', workspaceRoot: string}) => Promise<void>, listDir: (target: object) => Promise<Array<{name: string, type: string, target: {displayPath: string, targetKey: string}}>>, contains: (parent: object, child: object) => boolean}, subprocess: {spawn: (options: object) => {done: Promise<{exitCode: number}>, collected: {stdout?: {readFrom: (offset: number) => {text: string}}, stderr?: {readFrom: (offset: number) => {text: string}}}}}, dshHome: string}} options
 */
