...
/**
 * @param {{fs: {lstat: (path: string) => Promise<{type: string} | null>, resolve: (path: string) => Promise<{displayPath: string, targetKey: string}>, stat: (target: object) => Promise<{type: string, version?: unknown} | null>, readText: (target: object) => Promise<string>, writeText: (target: object, content: string, expected?: object) => Promise<void>, listDir: (target: object) => Promise<Array<{name: string, type: string, target: {displayPath: string, targetKey: string}}>>, contains: (parent: object, child: object) => boolean}, subprocess: {spawn: (options: object) => {done: Promise<{exitCode: number}>, collected: {stdout?: {readFrom: (offset: number) => {text: string}}, stderr?: {readFrom: (offset: number) => {text: string}}}}}, dshHome: string}} options
 */
export function createMarketRuntime({ fs, subprocess, dshHome }) {
  ...
  async function saveWorkspaceConfig(cwd, cfg) {
    ...
    const expected = existing
      ...
      : { kind: 'createIfAbsent' }
    await fs.writeText(target, JSON.stringify(normalizeWorkspaceConfig(cfg), null, 2), expected)
  }
   ...
   * @param {Awaited<ReturnType<typeof loadConfig>>} cfg
   * @param {Awaited<ReturnType<typeof loadWorkspaceConfig>>} workspace
   * @param {string} pluginKey
   * @param {T[]} skills
   * @returns {T[]}
   */
  function enabledPluginSkills(cfg, workspace, pluginKey, skills) {
    return skills.filter(/** @param {T} skill */ (skill) => pluginSkillEnabled(cfg, workspace, pluginKey, skill.fullName))
  }
   ...
   * @returns {T[]}
   */
  function enabledStandaloneSkills(cfg, workspace, skills) {
    return skills.filter(/** @param {T} skill */ (skill) => standaloneSkillEnabled(cfg, workspace, skill.fullName))
  }
  ...
  async function scanEffectivePluginSkills(cfg, workspace, market, entry) {
    ...
    if (!pluginEnabled(cfg, workspace, pluginKey)) return []
    const marketDir = marketsDir + '/' + market.id
    const pluginDir = await resolveDirectoryWithin(marketDir, entry.source)
    if (!pluginDir) return []
    const meta = await readPluginMeta(pluginDir, entry)
    return enabledPluginSkills(cfg, workspace, pluginKey, await scanPluginSkills(market, entry, pluginDir, meta))
  }
  ...
  async function collectSkills(options = {}) {
    ...
    for (const market of cfg.markets) {
      ...
      result.push(...enabledStandaloneSkills(cfg, workspace, await scanStandaloneSkills(market, mp)))
    }
    return result
  }
