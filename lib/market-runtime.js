...
/**
 * @typedef {{displayPath: string, targetKey: string}} FsTarget
 * @typedef {{type: 'file' | 'directory' | 'symlink' | 'other'}} FsStat
 * @typedef {{name: string, type: FsStat['type'], target: FsTarget}} FsDirectoryEntry
 * @typedef {{
 *   resolve: (path: string) => Promise<FsTarget>,
 *   stat: (target: string | FsTarget) => Promise<FsStat | null>,
 *   readText: (target: string | FsTarget) => Promise<string>,
 *   writeText: (target: string | FsTarget, content: string) => Promise<void>,
 *   listDir: (target: string | FsTarget) => Promise<FsDirectoryEntry[]>,
 *   contains: (parent: string | FsTarget, child: string | FsTarget) => boolean
 * }} FsAdapter
 * @typedef {{readFrom: (offset: number) => {text: string}}} CollectedText
 * @typedef {{done: Promise<{exitCode: number}>, collected: {stdout?: CollectedText, stderr?: CollectedText}}} SpawnedProcess
 * @typedef {{spawn: (options: {argv: string[], cwd: string, stdio: object, graceMs: number, env: Record<string, string>}) => SpawnedProcess}} SubprocessAdapter
 ...
 * @typedef {{markets: MarketEntry[], installed: Record<string, InstalledPlugin>, disabledSkills: Record<string, boolean>, enabledStandaloneSkills: Record<string, boolean>, hookApprovals: Record<string, HookApproval>}} MarketConfig
 * @typedef {{name: string, source: string, unsupported: boolean, description: string, sourceType?: string}} MarketplacePluginEntry
 * @typedef {{name: string, description: string, plugins: MarketplacePluginEntry[]}} MarketplaceManifest
 * @typedef {{root: 'market' | 'plugin', path: string}} SkillSource
 * @typedef {{title: string, description: string, skillSources: SkillSource[], hookConfigs: {codex: object | null}}} PluginMeta
 * @typedef {{data: Record<string, string | number | boolean>, content: string}} ParsedSkillDocument
 * @typedef {{skillName: string, description: string, whenToUse: string | null, fullName: string, path: string, resourceDir: string}} SkillRecord
 * @typedef {object} MarketRuntime
 * @property {{baseDir: string, marketsDir: string, configPath: string, generatedHooksDir: string, hookDataDir: string}} paths
 * @property {() => Promise<void>} prepare
 * @property {(argv: string[], cwd: string) => Promise<string>} runProc
 * @property {(dir: string) => Promise<void>} ensureDir
 * @property {() => Promise<MarketConfig>} loadConfig
 * @property {(config: MarketConfig) => Promise<void>} saveConfig
 * @property {(parentDir: string, childPath: string) => Promise<string | null>} resolveDirectoryWithin
 * @property {(marketDir: string, marketRepo: string | undefined) => Promise<MarketplaceManifest | null>} parseMarketplace
 * @property {(pluginDir: string, fallback: MarketplacePluginEntry | {name?: string, description?: string} | string) => Promise<PluginMeta>} readPluginMeta
 * @property {(rootDir: string, path: string) => Promise<ParsedSkillDocument | null>} readSkillFileWithin
 * @property {(market: MarketEntry, entry: MarketplacePluginEntry, pluginDir: string, meta: PluginMeta) => Promise<SkillRecord[]>} scanPluginSkills
 * @property {(market: MarketEntry, marketplace: MarketplaceManifest | null) => Promise<SkillRecord[]>} scanStandaloneSkills
 * @property {() => Promise<SkillRecord[]>} collectSkills
 */
...
/**
 * Builds the filesystem-backed market discovery and persistence runtime.
 *
 * @param {{fs: FsAdapter, subprocess: SubprocessAdapter, dshHome: string}} options
 * @returns {MarketRuntime}
 */
