...
/**
 * @typedef {{found: boolean, declared: boolean, configs: Array<{source: string, config: Record<string, unknown>}>, fingerprint?: string, pluginRoot?: string, error: string | null}} HookConfigInfo
 * @typedef {{dispose: () => void | Promise<void>, config?: {configPath?: string}}} HookFiber
 * @typedef {{lstat: (path: string) => Promise<{type: string} | null>, resolve: (path: string) => Promise<{displayPath: string, targetKey: string}>, contains: (parent: object, child: object) => boolean, stat: (target: object) => Promise<{type: string} | null>, readText: (target: object) => Promise<string>, writeText: (target: object, content: string) => Promise<void>}} HookFs
 * @typedef {ReturnType<typeof import('./market-runtime.js').createMarketRuntime>} MarketRuntime
 * @typedef {object} CodexHookManager
 * @property {boolean} available
 * @property {() => void} start
 * @property {() => Promise<void>} reconcile
 * @property {(marketDir: string, pluginDir: string, meta: object) => Promise<HookConfigInfo>} inspectCodexHooks
 * @property {(key: string) => Promise<void>} disposeHookFibers
 * @property {(marketId: string) => Set<string>} activeKeysForMarket
 * @property {(marketId: string) => void} suspendMarket
 * @property {(marketId: string) => void} resumeMarket
 * @property {(key: string) => boolean} isActive
 * @property {(key: string) => string | null} runtimeError
 * @property {(key: string) => void} clearRuntimeError
 * @property {(marketId: string) => Promise<void>} revokeMarketHookApprovals
 * @property {(market: {id: string, repo: string}, activeKeys: ReadonlySet<string>) => Promise<void>} restoreActiveMarketHookApprovals
 */

// Keep this adapter protocol-specific; a Claude bridge can live beside it.
/**
 * Creates the lifecycle adapter for approved Codex Hook configurations.
 *
 * @param {{ctx: import('@deepseek-ai/cordis').Context, fs: HookFs, bridge: unknown | null, runtime: MarketRuntime}} options
 * @returns {CodexHookManager}
 */
