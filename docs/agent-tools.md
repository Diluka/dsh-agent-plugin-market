...
在「设置 → 插件 → 插件配置 → Agent 插件市场」中展开「功能」分组：

- **启用工具**：默认开启，控制三个代理市场工具的注册；关闭后，市场管理、技能加载和 hooks 仍可使用。
- **注入系统提示词**：默认开启，可单独关闭。工具未启用时默认也禁用注入，即使保留此项的开启偏好，也不会注入工具说明；home 路径会话同样不注入。

设置由 DSH 的 `agent-plugin-market` namespace 保存并实时生效，配置键为 `tools` 和 `systemPrompt`。提示词简要引导代理先查询市场状态、使用返回的精确标识，再设置工作区的插件或技能三态覆盖。它不包含市场清单或技能正文。
...
设置某个市场技能在当前工作区中的启用模式。工具会根据 `full_name` 自动判断目标是插件技能还是独立技能，并写入对应覆盖组。插件技能的 `enabled` 覆盖会独立生效：即使插件自身继承全局且未安装，该技能仍会在本工作区加载；只有插件级 `disabled` 才会让该插件下所有技能在该工作区失效。
