...
- **Codex hooks**：发现插件 manifest 的 hooks 配置；用户逐插件显式确认后，以 `@deepseek-ai/dsh-hooks-codex` 注册受支持的 hook 点
...
安装完成后**重启 DeepSeek Harness**，设置 → 插件市场 即可使用。DSH 自带运行时包通过 peer dependencies 由当前 DSH profile 提供；市场包只安装 hooks bridge 与其协议包。
...
5. **Codex hooks 授权**：安装的 Codex 插件检测到 hooks 配置后默认关闭。开关需要再次点击确认；可更新市场拉取到任何新提交时，市场会自动停止该市场的 hooks，等待新的显式确认。
